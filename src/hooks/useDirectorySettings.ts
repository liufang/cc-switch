...
> = {
  ...
  sohocode: { key: "sohocode", defaultFolder: ".sohoCode" },
};
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("sohocode"),
          ...
          computeDefaultConfigDir("sohocode"),
        ]);
        ...
        setResolvedDirs({
          ...
          sohocode: sohocodeDir || defaultsRef.current.sohocode,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        sohocode: overrides?.sohocode ?? defaultsRef.current.sohocode,
      });
    },
    [],
  );
